    ...
    stage('Artifact Push') {
     steps {	          
	   //  sh(script: 'mvn  -version')-Dtest=!SeleniumTest
             sh(script: 'mvn deploy -Drevision=01 -Dtest=!SeleniumTest')
        echo 'Artifact Push...'
     }
   }
	      ...
	      steps{
		   ...
		   script {
               sshagent (credentials:['deployserver']) { 
                sh 'ssh -o StrictHostKeyChecking=no ec2-user@172.31.9.220 "killall -9 java; rm -rf gs-spring-boot-1.0.1.jar; ls -ltr; ps -ef |grep java; wget http://65.1.231.149:8081/repository/spring-boot/org/springframework/gs-spring-boot/1.0.1/gs-spring-boot-1.0.1.jar; "'		
...
/*     stage('Deploy to Production') {
       steps {	
	     ...
	     sh 'ssh -o StrictHostKeyChecking=no ec2-user@172.31.7.15 "killall -9 java; rm -rf gs-spring-boot-1.0.1.jar; ls -ltr; ps -ef |grep java ; wget http://65.1.231.149:8081/repository/spring-boot/org/springframework/gs-spring-boot/1.0.1/gs-spring-boot-1.0.1.jar;"'		
